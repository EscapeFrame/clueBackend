...
import hello.cluebackend.assignment.repository.AssignmentRepository;
...
public class AssignmentService {
  ...
  public List<GetAllAssignment> findAllAssignment(Long userId, Long classId) {
    try{
      validated(userId, classId);
      List<Assignment> assignments = assignmentRepository.findAllByclassId(classId);
      List<GetAllAssignment> result = assignments.stream()
              .map(a -> new GetAllAssignment(
                      a.getAssignmentId(),
                      a.getTitle(),
                      a.getStartDate(),
                      calculateRemainingTime(a.getStartDate(),a.getEndDate())
              ))
              .toList();
  ...
  public void deleteAssignment(Long userId,Long classId, Long assignmentId) {
    try{
      UserEntity user = validated(userId, classId);
...
//  }
  ...
  private String calculateRemainingTime(LocalDateTime startDate, LocalDateTime dueDate) {
