package hello.cluebackend.domain.student.classroom.service;

import hello.cluebackend.domain.student.classroom.domain.ClassRoom;
import hello.cluebackend.domain.student.classroom.domain.repository.ClassRoomRepository;
import hello.cluebackend.domain.student.classroom.controller.dto.ClassRoomAllInfoDto;
...
import hello.cluebackend.domain.student.classroomuser.domain.repository.ClassRoomUserRepository;
...
//                .filter(cu -> cu.getUser().getRole() == Role.STUDENT)
    ...
    public ClassRoomAllInfoDto getAllInfo(UUID classId) {
        ...
        List<DirectoryAllInfoDto> directoryDtoList = classRoom.getDirectoryList().stream()
                .sorted(Comparator.comparingInt(Directory::getDirectoryOrder)) // directoryOrder 기준 정렬
                .map(directory -> {
                    List<DocumentAllInfoDto> documentDtoList = directory.getDocumentList().stream()
                            .sorted(Comparator.comparing(Document::getCreatedAt)) // createdAt 기준 정렬
